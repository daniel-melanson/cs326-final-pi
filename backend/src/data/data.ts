...
/*
  ...
  */
  ...
  let rawEvents = JSON.parse(fs.readFileSync('src\\data\\rawEvents.json').toString());
  ...
  */
  ...
  let calendarInfo = JSON.parse(fs.readFileSync('src\\data\\calendarInfo.json').toString());
  ...
  for (let i = 0; i < rawEvents.length; i++) {
    ...
    if (owner && room && calendarTime && calendarTime['rsrv']) {
      ...
      if (reservationTime && reservationTime['rsrv_start_dt']) {
        let startTime = new Date(reservationTime['rsrv_start_dt']);
        let endTime= new Date(reservationTime['rsrv_end_dt']);
...
async function updateRooms() {
  ...
  for (let index = 0; index < rawRooms.length; index++) {
    ...
    const roomIndex = Math.max(rawRooms[index][1].indexOf(' room'),
